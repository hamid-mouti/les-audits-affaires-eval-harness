...
class BatchEvaluator:
    ...
    def process_all_requests(self, max_requests: Optional[int] = None, simulate: bool = False) -> Dict:
        ...
        for i, request in enumerate(pending_requests, 1):
            ...
            try:
                # Evaluate the model
                if simulate:
                    scores = self.simulate_evaluation(request["model_name"], request["model_provider"])
                else:
...
def main():
    ...
    parser.add_argument("--simulate", action="store_true", help="Use simulated evaluation scores for testing")
    parser.add_argument("--clear-requests", action="store_true", help="Clear all requests before processing (dangerous!)")
    ...
    results = evaluator.process_all_requests(
    ...
    if results['successful'] > 0:
        ...
        if not args.dry_run:
            print("📊 Check the leaderboard: https://huggingface.co/spaces/legmlai/laal-leaderboard")
