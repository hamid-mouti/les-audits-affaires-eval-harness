...
class ResultsUploader:
    ...
    def find_request_id(self, model_name: str, provider: str) -> Optional[str]:
        ...
        for request in requests_dataset:
            if (request["model_name"].lower() == model_name.lower() and 
                request["model_provider"].lower() == provider.lower()):
                return request["request_id"]
        
        # If not found, create a new request entry
        print(f"⚠️  No matching request found for {model_name} ({provider}). Creating new request...")
    ...
    def create_request_entry(self, model_name: str, provider: str) -> str:
        ...
        try:
            ...
            updated_data = {key: requests_dataset[key] + [new_request[key]] 
        ...
        try:
            ...
            for sample in data:
                if 'evaluation' in sample and 'scores' in sample['evaluation']:
                    ...
                    scores['consequences_non_conformite'] += eval_scores.get('consequences_non_conformite', 0)
    ...
    def upload_results(self, model_name: str, provider: str, scores: Dict, request_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            existing_results = [r for r in results_dataset if r["model_name"] == model_name and r["model_provider"] == provider]
            ...
            if existing_results:
                ...
                updated_data = {key: results_dataset[key] + [new_result[key]] 
...
def main():
    ...
    if args.results_file:
        ...
        root_results_file = project_root / f"results_{args.provider}_{args.model_name.replace('-', '_')}.json"
        if root_results_file.exists():
            scores = uploader.parse_results_file(str(root_results_file), args.model_name, args.provider)
        else:
            # Try results directory
            results_dir = project_root / "results" / args.model_name.replace('-', '_')
            if results_dir.exists():
                scores = uploader.parse_results_directory(str(results_dir), args.model_name, args.provider)
            else:
                print(f"❌ No results found for {args.model_name}. Please specify --results_file or --results_dir")
                return
